...
// filterAssignments takes a list of assignments and filters them into 3 groups. The first is the assigned tasks which
// are tasks that have been finally assigned to a host. The second is retryable tasks which are tasks that may or may
// not have a host assigned, but which should be retried in any case to see if there is a host that is better for them.
// The third is unassigned tasks which are tasks that we have not been able to assign a host to and which we should not
// retry.
...
func (e *engine) findUnusedHosts(assigned, retryable []*models.Assignment,
	...
	// For each offer determine if any tasks were assigned to it.
...
func (e *engine) placeAssignmentGroup(ctx context.Context, filter *hostsvc.HostFilter, assignments []*models.Assignment) {
	...
	deadline := time.Now().Add(e.config.MaxPlacementDuration)
	for len(assignments) > 0 {
		// Try to get some offers
		hosts := e.offerService.Acquire(
			ctx, e.config.FetchOfferTasks, e.config.TaskType, filter)
		existing := e.findUsedHosts(assignments)
		now := time.Now()
		for !e.pastDeadline(now, assignments) && now.Before(deadline) && len(hosts) == 0 && len(existing) == 0 {
		...
		// PlaceOnce the tasks on the offers by delegating to the placement strategy.
