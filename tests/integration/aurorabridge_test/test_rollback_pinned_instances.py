...
def test__auto_rollback_with_pinned_instances__stopped_instances(client):
    ...
    pinned_req = get_job_update_request('test_dc_labrat_large_job_new_config.yaml')
    pinned_req.settings.updateOnlyTheseInstances = set([api.Range(first=i, last=i)
                                                        for i in update_instances_2])
    ...
    for t in res.tasks:
        ...
        if t.assignedTask.instanceId in (stop_instances - bad_update_instances):
