...
from peloton_client.pbgen.peloton.private.resmgrsvc import resmgrsvc_pb2 as resmgr
...
class Task(object):
    ...
    def wait_for_pending_state(self, goal_state):
        ...
        while attempts < self.job.config.max_retry_attempts:
            try:
                ...
                if state != new_state:
                    log.info('%s transitioned to state %s', self.mesos_task_id,
                             new_state)
                state = new_state
                if state == goal_state:
        ...
        if state_transition_failure:
            log.info('%s goal_state:%s current_state:%s attempts: %s',
                     self.mesos_task_id, goal_state, state, str(attempts))
            assert False

        if attempts == self.job.config.max_retry_attempts:
            log.info('%s max attempts reached to wait for goal state',
                     self.mesos_task_id)
            log.info('%s goal_state:%s current_state:%s', self.mesos_task_id,
                     goal_state, state)
            assert False

        end = time.time()
        elapsed = end - start
        log.info('%s state transition took %s seconds', self.mesos_task_id, elapsed)
    ...
    def _get_active_tasks(self):
        request = resmgr.GetActiveTasksRequest(
