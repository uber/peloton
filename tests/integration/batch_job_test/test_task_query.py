...
def query_request(job_id, task_state=None, pagination=None, names=None, hosts=None):
...
def query_response(request):
    ...
    task_query_response = client.task_svc.Query(
        request, metadata=client.jobmgr_metadata,
        timeout=10)
...
def is_sorted(sort_results, sort_by_field, sort_order):
    ...
    return all(compare(_get_field_value(sort_results[i], sort_by_field),
...
@pytest.mark.parametrize('task_test_fixture', [([('SUCCEEDED', 3)])],
                         indirect=True)
...
@pytest.mark.parametrize('task_test_fixture', [([('RUNNING', DEFAUILT_TASKS_COUNT)])],
                         indirect=True)
...
@pytest.mark.parametrize('task_test_fixture', [([('FAILED', DEFAUILT_TASKS_COUNT)])],
                         indirect=True)
...
@pytest.mark.parametrize('task_test_fixture', [([('SUCCEEDED', 1), ('FAILED', 2)])],
                         indirect=True)
...
@pytest.mark.parametrize('task_test_fixture', [([('SUCCEEDED', 3), ('FAILED', 2)])],
                         indirect=True)
def test__task_query_by_name(task_test_fixture):
    ...
    req_by_name_2 = query_request(task_test_fixture, names=['task-2', 'task-3'])
    response = query_response(req_by_name_2)
    assert len(response.records) == 2
    for resp_task in response.records:
        assert resp_task.config.name == 'task-2' or resp_task.config.name == 'task-3'
...
@pytest.mark.parametrize('task_test_fixture', [([('SUCCEEDED', 3)])], indirect=True)
