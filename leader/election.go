...
const (
	...
	ttl = 15 * time.Second
	// zkConnErrRetry how long to wait before restarting campaigning for leadership on connection error
...
func NewCandidate(cfg ElectionConfig, scope tally.Scope, role string, nomination Nomination) (Candidate, error) {
	log.WithFields(log.Fields{"id": nomination.GetID(), "role": role}).Debug("Creating new Candidate")

	if role == "" {
		return nil, errors.New("You need to specify a role to campaign for that isnt the empty string")
	}
...
func (el *election) Start() error {
	...
	go func() {
		for el.running {
			err := el.waitForEvent()
			if err != nil {
				log.WithFields(log.Fields{"role": el.role}).Errorf("Failure running election; retrying: %v", err)
			}
...
// waitForEvent handles events like this host gaining or losing leadership, or a connectivity error occurring.
// NOTE: this function blocks until an event is handled from either the error channel or the event channel. It should
// be called by a wrapper function that handles retries
func (el *election) waitForEvent() error {
	...
	for {
		...
		case isElected := <-electionCh:
			if isElected {
				log.WithFields(log.Fields{"id": el.nomination.GetID(), "role": el.role}).Info("Leadership gained")
				el.metrics.GainedLeadership.Inc(1)
				el.metrics.IsLeader.Update(1)
				err := el.nomination.GainedLeadershipCallback()
				if err != nil {
					log.WithFields(log.Fields{"id": el.nomination.GetID(), "role": el.role, "error": err}).Error("GainedLeadershipCallback failed")
					return err
				}
			} else {
				log.WithFields(log.Fields{"id": el.nomination.GetID(), "role": el.role}).Info("Leadership lost")
				el.metrics.LostLeadership.Inc(1)
				el.metrics.IsLeader.Update(0)
				err := el.nomination.LostLeadershipCallback()
				if err != nil {
					log.WithFields(log.Fields{"id": el.nomination.GetID(), "role": el.role, "error": err}).Error("LostLeadershipCallback failed")
					return err
				}
			}
		case err := <-errCh:
			if err != nil {
				log.WithFields(log.Fields{"role": el.role, "error": err}).Error("Error participating in election")
				el.metrics.Error.Inc(1)
				return err
			}
			// just a shutdown signal from the docker/leadership lib, we can propogate this and let the caller decide
			// if we should continue to run, or terminate
			return nil
		}
	}
}
...
// NOTE: dont call this more than once, or you will panic trying to close a closed channel
...
func (el *election) IsLeader() bool {
	el.Lock()
	defer el.Unlock()
	// interestingly, the candidate reports leader even if we have resigned, so gate delegating to isLeader on whether we are actively
	// campaigning for the leadership
	return el.running && el.candidate.IsLeader()
}

// leaderZkPath returns the full ZK path to the leader node given a election config (the path root) and a component
