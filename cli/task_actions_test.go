...
// TestClientTaskGetEventsAction tests getting task events
func (suite *taskActionsTestSuite) TestClientTaskGetEventsAction() {
	c := Client{
		Debug:      false,
		taskClient: suite.mockTask,
		dispatcher: nil,
	...
	tt := []struct {
		debug bool
		req   *task.GetEventsRequest
		resp  *task.GetEventsResponse
		err   error
	}{
		{
			req: &task.GetEventsRequest{
			...
			resp: &task.GetEventsResponse{
				Result: []*task.GetEventsResponse_Events{
					{
			...
			resp: &task.GetEventsResponse{
				Error: &task.GetEventsResponse_Error{
					EventError: &task.TaskEventsError{
						Message: "task get events failed",
					},
				},
			},
			err: nil,
		...
		},
		{
			...
			resp: nil,
			err:  errors.New("task get events failed"),
		},
	}
	...
	for _, t := range tt {
		c.Debug = t.debug
		suite.mockTask.EXPECT().
			GetEvents(gomock.Any(), t.req).
			Return(t.resp, t.err)

		if t.err != nil {
			suite.Error(c.TaskGetEventsAction(jobID.Value, instanceID))
		} else {
			suite.NoError(c.TaskGetEventsAction(jobID.Value, instanceID))
		}
	}
}
